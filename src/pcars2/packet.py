    ...
    def __init__(self, buildVersion, sequenceNumber, packetType, buf):
        ...
        if hasattr(self.__class__, "STRUCTURE"):
            try:
                ...
                print(
                    "Could not decode packet: " + str(self.sequenceNumber) + ": " + str(PACKET_TYPES[self.packetType])
                )
...
class TelemetryData(Packet):
    ...
    def __init__(self, buildVersion, sequenceNumber, packetType, buf):
        super(TelemetryData, self).__init__(
            buildVersion, sequenceNumber, packetType, buf
        )  # everything up to tyre information
        self._data["tyres"] = [{}, {}, {}, {}]

        for datapoint in TelemetryData.TYRES_STRUCTURE:
            self._forEachTyre(datapoint, buf)
        #
        self._data.update(TelemetryData.STRUCTURE_2.read_dict(buf))

        # self._forEachTyre(TelemetryData.TYRES_STRUCTURE_2, buf)
        # for i in Tyres:
        # 	 self._data["tyres"][i.value][TelemetryData.TYRES_STRUCTURE_2[2]] = thisField.read_dict(buf)[datapoint[2]]
...
class RaceData(Packet):
    ...
    STRUCTURE = binio.new(
        [
            ...
            (64, binio.types.t_string_utf8, "sTrackLocation"),
            (64, binio.types.t_string_utf8, "sTrackVariation"),
            (64, binio.types.t_string_utf8, "sTranslatedTrackLocation"),
            (64, binio.types.t_string_utf8, "sTranslatedTrackVariation"),
...
class ParticipantsData(Packet):
    ...
    STRUCTURE = binio.new([(1, binio.types.t_uint, "ParticipantsChangedTimestamp")])
...
class ParticipantVehicleNamesData(Packet):
    ...
    def __init__(self, buildVersion, sequenceNumber, packetType, buf):
        ...
        for _ in range(0, 60):
            # FIXME UnicodeDecodeError: 'ascii' codec can't decode byte 0xba in position 2: ordinal not in range(128)
            p = ParticipantVehicleNamesData.VEHICLE_CLASS_INFO.read_dict(buf)
...
NOTE: If there are less than 16 participants in a race the name and
      ...
      Use numParticipants from TelemetryData to work out how many
